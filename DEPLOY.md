# Deploy Fulcru to Railway

## Prerequisites

- Railway CLI installed (`npm i -g @railway/cli`) or use the Railway web UI
- A Neon Postgres project
- Google OAuth credentials (or you can ship without them and use email/password)
...
2. **Init the project from this repo**
   ...
   railway init
   ```
   Choose **Empty Project**. Name it `fulcru`.
...
4. **Set environment variables**, copy these from your `.env.local`, but set `BETTER_AUTH_URL` and `NEXT_PUBLIC_APP_URL` to the Railway public URL once it's assigned (see step 6).
   ...
   ```sh
   railway variables \
     --set "DATABASE_URL=<your-neon-pooled-connection-string>" \
...
5. **First deploy**
   ...
   railway up
   ```
   Railway builds via Nixpacks (auto-detects Next.js). Wait for the first build (~3 to 5 min).

6. **Get the public domain**
   ```sh
   railway domain
   ```
   Copy the assigned domain (e.g. `fulcru-production.up.railway.app`).
